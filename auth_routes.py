...
# Initialize the API Router with a prefix and tags for grouping related routes
auth_router=APIRouter(
    prefix='/auth',
    tags=['auth']
)

# Create a new database session bound to the engine
session=Session(bind=engine)

# GET route that returns a "Hello World" message
@auth_router.get('/')
 ...
 # Check if the email already exists in the database
    ...
    if db_email is not None:
        return HTTPException (
            ...
            detail = "User with the email already exisits"
            )
    ...
    if db_username is not None:
        return HTTPException (
            status_code=status.HTTP_400_BAD_REQUEST, 
            detail = "User with the username already exisits"
            )
    ...
    # Add the new suser to the database session
